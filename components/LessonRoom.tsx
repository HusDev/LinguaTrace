...
export function LessonRoom() {
  ...
  async function capture(fromWhiteboard = panel === "whiteboard") {
    ...
    /* Three different things used to produce the same silence: no board yet, a
       board with nothing on it, and an export that threw. The last one was the
       worst - `void capture()` swallows a rejection, so a failed export left no
       card, no message and nothing in the console. Each says what happened. */
    if (onBoard && !boardApi.current) {
      setStatus("The whiteboard is still starting. Try again in a moment.");
      return;
    }
    ...
    try {
      ...
      setStatus(
        onBoard
          ? `The whiteboard could not be exported: ${detail}`
          : `The camera frame could not be copied: ${detail}`,
      );
      return;
    }
    ...
    if (!dataUrl) {
      setStatus(
        onBoard
          ? "Nothing drawn on the whiteboard yet - draw something first."
    ...
    let shared: string;
    try {
      shared = await shrinkCapture(dataUrl);
    } catch (error) {
      const detail = error instanceof Error ? error.message : String(error);
      setStatus(`The capture could not be prepared: ${detail}`);
      return;
    }
  ...
  /* `flex-1` and `h-screen` disagree, and on a flex item flex-basis wins:
     `flex: 1 1 0%` makes the height come from the content, so the page grew
     with the transcript however many `min-h-0`s sat below it. `lg:flex-none`
     hands the sizing back to `h-screen`, which is the only thing in this chain
     that knows how tall the window is. */
  return (
    <main className="flex-1 flex flex-col gap-3 p-3 lg:flex-none lg:h-screen lg:overflow-hidden">
      {isDesktop && (
      <header className="flex flex-wrap items-center gap-2 lg:gap-3 px-1 shrink-0">
        <Logo className="h-9 w-9 shrink-0 drop-shadow-[0_2px_6px_rgba(0,0,0,0.35)]" />
      ...
      {/* The row is pinned to the height of the grid, not to its content.
          Without `grid-rows`, an implicit row is `auto` and sizes to whatever
          is inside it - so the rail grew with the transcript, pushed past the
          bottom of the page, and the transcript's own scrollbar never had a
          height to scroll within. `min-h-0` on the children cannot help while
          the row itself is free to grow. */}
      {isDesktop && (
      <div className="grid gap-3 lg:grid-cols-[320px_minmax(0,1fr)] lg:grid-rows-[minmax(0,1fr)] flex-1 min-h-0 overflow-hidden">
        <div className="hidden lg:flex min-h-0 min-w-0 flex-col overflow-hidden">
        <LiveRail
          notebook={notebook}
          previous={previous}
          phase={phase}
